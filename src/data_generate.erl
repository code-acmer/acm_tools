...
record_version_from_ets(GenerateDir) ->    
    write_version(GenerateDir, ets:tab2list(data_version)),
...
data_generate(GenerateDir, RecordName, TableName, IncludeFiles, FunConfList) ->
    ...
    case check_new_version(GenerateDir, TableName, Data) of
        true ->            
...
data_generate_with_db_data(Data, ModuleName, RecordName, IncludeFiles, FunConfList, GenerateDir) ->
    ...
    %% record info
    RecordFields = ?DATA_MOUDLE:get_fields(RecordName),
    FieldsWithPos = lists:zip(RecordFields, lists:seq(2, 1+length(RecordFields))),
    ...
    GenerateFunStrFun = 
        fun (#generate_conf{
                ...
                if 
                    ArgsConf =/= all ->
                        DefaultFunStr = lists:concat([FunName, "(", 
                                                      string:join(lists:duplicate(ArgsCount, "_"), ", "),
                                                      ") -> ", term_to_string(Default), ".\n"]),
