...
parse_arguments(CmdLine) ->
    io:format("~nparse_arguments: ~p~n", [opt_spec_list()]),
    case getopt:parse(opt_spec_list(), CmdLine) of
        {ok, {Opts, [Command | Args]}} ->
            io:format("Opts: ~p ~nCommand: ~p ~nArgs: ~p~n", [Opts, Command, Args]),
...
start() ->
    ...
    io:format("Data: ~p", [init:get_plain_arguments()]),
    {Command, Opts, Args} =
...
action(all, _Args, Opts) ->
    io:format("action: ~p~n", [table_to_record:tables(Opts)]),
